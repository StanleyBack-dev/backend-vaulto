import { AppException } from "@/common/exceptions/app-exception";
import { FinalizeSupportTicketUseCase } from "@/modules/support/application/use-cases/finalize-support-ticket.use-case";
import { AuthPermission } from "@/modules/auth/domain/enums/auth-permission.enum";
import { SupportCategory } from "@/modules/support/domain/enums/support-category.enum";
import { SupportTicketStatus } from "@/modules/support/domain/enums/support-ticket-status.enum";

function buildUseCase() {
  const authorizationService = {
    assertPermissionForUserId: jest.fn().mockResolvedValue(undefined),
  };

  const ticket = {
    idSupportMessage: "ticket-1",
    idUsers: "user-1",
    protocolNumber: 12,
    category: SupportCategory.DOUBT,
    message: "Preciso de ajuda",
    status: SupportTicketStatus.ANSWERED,
    adminReply: "Aqui está a resposta",
    createdAt: new Date("2026-08-13T12:00:00.000Z"),
  };

  const supportMessageRepository = {
    findTicketById: jest.fn().mockResolvedValue(ticket),
    finalize: jest.fn().mockResolvedValue({
      ...ticket,
      status: SupportTicketStatus.RESOLVED,
      finalizedByAdminId: "admin-1",
    }),
  };

  const supportTicketFinalizedEmailUseCase = {
    send: jest.fn().mockResolvedValue(undefined),
  };

  const usersById: Record<string, { name: string; email: string }> = {
    "user-1": { name: "Stanley", email: "stanley@example.com" },
    "admin-1": { name: "Admin Master", email: "admin@example.com" },
  };

  const userRepository = {
    findOne: jest
      .fn()
      .mockImplementation(({ where: { idUsers } }) =>
        Promise.resolve(usersById[idUsers] ?? null),
      ),
  };

  const useCase = new FinalizeSupportTicketUseCase(
    authorizationService as never,
    supportMessageRepository as never,
    supportTicketFinalizedEmailUseCase as never,
    userRepository as never,
  );

  return {
    useCase,
    authorizationService,
    supportMessageRepository,
    supportTicketFinalizedEmailUseCase,
  };
}

describe("FinalizeSupportTicketUseCase", () => {
  it("checks the manage-tickets permission and marks the ticket resolved by the calling admin", async () => {
    const { useCase, authorizationService, supportMessageRepository } =
      buildUseCase();

    const result = await useCase.execute("admin-1", "ticket-1");

    expect(authorizationService.assertPermissionForUserId).toHaveBeenCalledWith(
      "admin-1",
      AuthPermission.MANAGE_SUPPORT_TICKETS,
    );
    expect(supportMessageRepository.finalize).toHaveBeenCalledWith(
      "ticket-1",
      "admin-1",
    );
    expect(result.status).toBe(SupportTicketStatus.RESOLVED);
    expect(result.finalizedByName).toBe("Admin Master");
  });

  it("throws when the ticket does not exist", async () => {
    const { useCase, supportMessageRepository } = buildUseCase();
    supportMessageRepository.findTicketById.mockResolvedValueOnce(null);

    await expect(useCase.execute("admin-1", "missing")).rejects.toBeInstanceOf(
      AppException,
    );
  });

  it("emails the requester that the ticket was finalized", async () => {
    const { useCase, supportTicketFinalizedEmailUseCase } = buildUseCase();

    await useCase.execute("admin-1", "ticket-1");

    expect(supportTicketFinalizedEmailUseCase.send).toHaveBeenCalledWith(
      expect.objectContaining({
        to: "stanley@example.com",
        name: "Stanley",
        protocolNumber: 12,
        category: SupportCategory.DOUBT,
        adminReply: "Aqui está a resposta",
      }),
    );
  });
});
